...
## Note: Phase 0 has not run

This session ran Phase 1 directly. Phase 0's deliverables are still owed: the 8
`tests/test_engine.py` environment failures (missing gitignored
`third_party/argyll/3.3.0/ref/*.icm` + `C:\` fixture paths) are still **red**, not
skipped, and no coverage baseline exists yet. Baseline at phase start:
**800 collected — 790 passed, 8 env-failed, 2 skipped** (the two `DLC_COLORCAL`
lab-integration skips). After this phase: **830 collected — 820 passed, same 8
