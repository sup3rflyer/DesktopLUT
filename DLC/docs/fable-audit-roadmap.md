...
- **Updated:** 2026-09-21, Phase 9 ticket **T3 landed** on `claude/project-thread-djg427`
  (report addendum: `docs/audits/fable/phase-9.md` §5b). `state.get` mhc entries now carry
  `correction_grayscale {enabled, point_count, points, deviations}`, so the Design-B grayscale-wb
  revert can restore the user's own curve instead of clearing to identity. The restore path was
  dead code until now and re-bridges the curve through the SDR bridge — verified idempotent and
  pinned. The honesty tell now separates "no prior correction" from "this build cannot tell me".
  **MSVC build and hardware behaviour unverified** — HW-10. One discrepancy found and deliberately
  NOT fixed (the `correctionGrayscale.enabled` ⇄ `layers.grayscale` mirror trips DLC's neutral
  audit on resume): see §5b "Found while doing T3". T1 and T4 remain open.
- **Updated:** 2026-09-21, Phase 9 tickets **T1 and T4 landed** on `claude/project-thread-djg427`
  (report addendum: `docs/audits/fable/phase-9.md` §5c) — **the Phase 9 C++ ticket list is now
  closed (T1–T4)**. `state.get` reports `contract_version` (`kCalibrationContractVersion`), so the
  version handshake can finally fire; the `CPP_TICKETED_RESULT_KEYS` allowlist is empty and the
  shape test enforces every advertised key. The dead `maintenance.verify_mhc` branch is gone, with
  a general pin against any method Dispatch already serves off-thread reappearing in the GUI table.
  **MSVC build unverified** — HW-11.
...
| HW-11 | Phase 9 T1 + T4: on the box, confirm the build, that `state.get` reports `contract_version: 1` with no preflight mismatch, and that `maintenance.verify_mhc` still answers over the pipe | Phase 9 |
| HW-10 | Phase 9 T3 (`correction_grayscale` in `state.get`): on the box, confirm a run with an existing correction grayscale reads it back over the pipe, and that a grayscale touch-up followed by `revert` restores THAT curve instead of clearing to identity | Phase 9 |
