...
def test_hlsl_and_cpp_carry_the_zone_rule():
    ...
    assert len(fields) == 76 and fields[72:76] == ["boostRule", "boostMeanGamma", "boostMeanThresh", "_padB1"]
    ...
    assert "FALD_CB_BYTES = 304" in h and "FALD_BOOST_RULE_DIM = 0;" in h and "FALD_BOOST_RULE_MEAN = 1;" in h
    assert "float boostMeanGamma = 0.62f, boostMeanThresh = 0.0693f;" in h            # = FaldParams / panelfile defaults
    assert "u[72] = p.boostRule; f[73] = p.boostMeanGamma; f[74] = p.boostMeanThresh; u[75] = 0u;" in c
